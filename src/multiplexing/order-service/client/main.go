...
import (
	...
	"io"
	"log"
	pb "multiplexing/order-service/order-service-gen"
	"time"
...
func main() {
	...
	// 使用创建的gRPC连接来建立OrderManagement客户端。
	...
	// Get Order
	retrievedOrder, err := orderManagementClient.GetOrder(ctx, &wrappers.StringValue{Value: "106"})
	log.Print("GetOrder Response -> : ", retrievedOrder)

	// Search Order
	searchStream, _ := orderManagementClient.SearchOrders(ctx, &wrappers.StringValue{Value: "Google"})
	for {
